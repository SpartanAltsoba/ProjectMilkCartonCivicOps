import { runE2EIntegrationTests } from "./e2e-integration-test.js";
...
async function runTestSuite(options: TestRunnerOptions = {}) {
  ...
  try {
    if (apiOnly) {
      ...
      const results = await runAPITests(debug);
      return results;
    } else if (uiOnly) {
      console.log("Running UI tests only...\n");
      const { runUITests } = await import("./e2e-integration-test.js");
      const results = await runUITests();
      return results;
    } else {
      console.log("Running full E2E integration test suite...\n");
      const results = await runE2EIntegrationTests();
      return results;
    }
  } catch (error) {
